...
async function run() {
  const { data, error } = await supabase.from('videos_feed').select('*').limit(1);
  if (error) {
    console.error('Error fetching video:', error);
  } else {
    console.log('Video columns:', data[0] ? Object.keys(data[0]) : 'No records found');
    console.log('Video record:', data[0]);
  }
}
