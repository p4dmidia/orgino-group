...
export default function AdminModeration() {
  ...
  const fetchComments = async () => {
    ...
    try {
      // BUSCA DIRETA NO BANCO (Resolve o erro 404 da Edge Function)
      ...
      const formatted = data.map((c: any) => ({
        ...
        lesson_title: c.lessons?.title || 'Aula'
      }));

      setComments(formatted);
    } catch (err: any) {
      console.error('Fetch Error:', err);
      toast.error("Erro ao carregar comentários reais: " + err.message);
    } finally {
      setLoading(false);
    }
  };

  const handleAction = async (id: string, action: 'approve_comment' | 'delete_comment') => {
    ...
    try {
      ...
      toast.success(action === 'approve_comment' ? "Aprovado!" : "Excluído!");
      await fetchComments();
    } catch (err: any) {
              ...
              comments.map((comment, index) => (
                ...
                >
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                       ...
                       <span className="flex items-center gap-2 bg-blue-500/10 text-blue-400 px-3 py-1 rounded-full text-[10px] font-black uppercase border border-blue-500/20">
                          <BookOpen size={12} /> {comment.lesson_title}
                       </span>
                       {comment.is_approved ? (
                         <span className="flex items-center gap-2 bg-emerald-500/10 text-emerald-500 px-3 py-1 rounded-full text-[10px] font-black uppercase border border-emerald-500/20">
                  ...
                  <div className="flex gap-3 mt-6 md:mt-0">
                    {!comment.is_approved && (
                      <button 
                        onClick={() => handleAction(comment.id.toString(), 'approve_comment')}
                        disabled={loading}
                    ...
                    <button 
                      onClick={() => handleAction(comment.id.toString(), 'delete_comment')}
                      disabled={loading}
